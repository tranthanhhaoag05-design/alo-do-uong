...
const HomePage = () => {
    ...
    // STATE MỚI: QUẢN LÝ THÔNG TIN KHÁCH HÀNG & MODAL ĐĂNG NHẬP
    ...
    // ==========================================
    useEffect(() => {
        // Tự động kiểm tra xem thiết bị này đã từng mua hàng chưa
    ...
    // HÀM BẤM NÚT "XEM & ĐẶT" (CHẶN CỬA KIỂM TRA ĐĂNG NHẬP)
    const handleOpenCart = () => {
        if (!customerName || !customerPhone) {
            setShowAuthModal(true); // Chưa có thông tin -> Mở bảng hỏi Tên/SĐT
        } else {
            setShowModal(true); // Có rồi -> Vào thẳng giỏ hàng
        }
    };

    // HÀM LƯU THÔNG TIN KHÁCH (LƯU VÀO ĐIỆN THOẠI)
    const handleSaveUserInfo = () => {
        if (!customerName.trim() || !customerPhone.trim()) {
            alert("Vui lòng nhập đầy đủ Tên và Số điện thoại để quán dễ liên hệ nhé!");
            return;
        }
        // Lưu vĩnh viễn vào bộ nhớ trình duyệt của khách
        localStorage.setItem('alo_customer_name', customerName);
        localStorage.setItem('alo_customer_phone', customerPhone);
        
        setShowAuthModal(false); // Đóng bảng đăng nhập
        setShowModal(true); // Mở giỏ hàng cho khách mua tiếp
    };

    // HÀM TẠO NỘI DUNG TIN NHẮN (ĐÃ THÊM THÔNG TIN KHÁCH)
    ...
    return (
        <div className="m-0 p-0" style={{ paddingBottom: '100px', backgroundColor: '#f8f9fa', minHeight: '100vh', fontFamily: "'Baloo 2', cursive" }}>
            ...
            {/* Float Giỏ Hàng (ĐỔI onClick GỌI HÀM KIỂM TRA ĐĂNG NHẬP) */}
            {cart.length > 0 && (
            ...
            {/* 🔥 MODAL ĐĂNG NHẬP THÔNG TIN (XUẤT HIỆN KHI CHƯA CÓ INFO) 🔥 */}
            {showAuthModal && (
